...
class TreeBehavior extends CNestedSetBehavior
{
    ...
    {
    	return $this->Owner->attributes[$this->_lftValue];
    }
    ...
    {
        return $this->Owner->attributes[$this->_rgtValue];
    }
    ...
    public function appendChild($node, $brother = null) 
    {
        ...
        if($this->getIsNewRecord())
            throw new Exception('You can\'t append a node to a parent that is not yet in the database. Add the parent node to the tree first using AppendChild or using an Insert* method.');
        try
        {        
        	$minleft = -1;
            $maxright = -1;
    ...
    public function getNestedTree($returnrootnode = true, $keyfield = null)
    {
        ...
        // !!!!! used to be 0 here, which is uncomfortable
