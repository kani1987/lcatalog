...
class TreeBehavior extends CNestedSetBehavior {
	...
	protected static function updateRegisteredNodeRange($class, $adjustment, $minLeft = -1, $minRight = 1, $maxLeft = -1, $maxRight = -1, $levelAdjustment = 0) {
		foreach (self :: $_registered as $node) {
			...
			if (($lft >= $minLeft || $minLeft == -1) && ($lft <= $maxLeft || $maxLeft == -1) && ($rgt >= $maxRight || $maxRight == -1) && ($rgt <= $maxRight || $minRight == -1) && $levelAdjustment != 0) {
	...
	public function getChildNodes() {
		...
		$criteria = $builder->createCriteria($this->_lftCol . " > ? AND " . $this->_rgtCol . " < ? AND " . $this->_lvlCol . " = ?", array (
			$lft,
			$rgt,
			$level +1
		));
	...
	public function getNextSibling() {
		...
		$criteria = $builder->createCriteria($cstring, array (
	...
	public function getParentNode() {
		...
		$criteria = $builder->createCriteria($cstring, array (
		$this->getLeftValue(), $this->getRightValue(), $this->getLevelValue() - 1));
		$criteria->limit = 1;
		$command = $builder->createFindCommand($this->Owner->getTableSchema(), $criteria);
		return $this->Owner->populateRecord($command->queryRow());
	}
	...
	public function getTree($returnrootnode = true) {
		...
		$params = array (
		$this->getLeftValue(), $this->getRightValue());
		$builder = $this->Owner->getCommandBuilder();
	...
	public function printTree($rootId = 0, $format = 'xml') {
		$sql = "SELECT node.id, CONCAT( REPEAT(' ', COUNT(parent.name) - 1), node.name) AS name, node.level " .
		...
		$root = simplexml_load_string($xmltext);
	...
	function nestify($arrs, $depth_key = 'level') {
		$xmltext = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<tree></tree>";
		$root = simplexml_load_string($xmltext);
	...
	function write_nodes($node) {
		echo '<pre>$node<br/>'; print_r($node->getAttribute('name')); echo '</pre>';
		$sql = "insert into tree_copy set 
		    id={$node['id']}, 
		    name='{$node->name}'
		    lft=$node->left, 
		    rgt=$node->right";
//		Yii::app()->db->createCommand($sql)->query();
	...
	public function xml2tree($xml) {
		...
		foreach ($p->node as $node)
			$this->set_left_right($node);
		foreach ($p->node as $node)
			$this->write_nodes($node);
	}
}